function [AltF_actual, vF, Alt, v, t, mpayload, Alpha, m,q,gamma,D,zeta,phi, inc,T,CL,L,inc_diff,xi,dvtot,m4,vexo,hs,Kn] = ThirdStageSim(alt0,gamma0,v0, phi0, xi0, zeta0, m0, auxdata)
...
% mHS = 46.2; % Heat Shield Mass MODIFIED HEAT SHIELD FOR PULLUP
...
exocond = false;
heatshield = true;
while gamma(i) >= 0 && Alt(end) > 20000 && Alt(end)
% iterate until trajectory angle drops to 0, as long as 150s has passed (this allows for the trajectory angle to drop at the beginning of the trajectory)
    ...
    if Alt(i) < 85000
        ...
        q(i) = 1/2*rho(i)*v(i)^2;
        P0(i) = ppval(auxdata.interp.P0_spline, Alt(i)); % Pressure (Pa), in atmosphere
        T0(i) = ppval(auxdata.interp.T0_spline, Alt(i)); % Temperatue (K), in atmosphere
    else
        c(i) = ppval(auxdata.interp.c_spline, 85000);
        rho(i) = 0;
        q(i) = 0;
        P0(i) = ppval(auxdata.interp.P0_spline, 85000); % Pressure (Pa), in atmosphere
        T0(i) = ppval(auxdata.interp.T0_spline, 85000); % Temperatue (K), in atmosphere
        exocond = true;
    end
...
%     Alt(i+1) = r(i+1) - r_E;
    ...
    Alt(i+1) = Alt(i) + rdot(i)*dt + (rdot(i) - rdot(i-1))/2*dt;
...
mpayload = m4 - (auxdata.Stage3.mTot - mHS - mEng-100)*0.079 -mEng; % 7.9% structural mass used, from falcon 1 guide, second stage masses with no fairing or engine
